...
export default class Configurator extends PageManager {
  ...
  constructor() {
    ...
    this.boundMountingChange = this.handleMountingChange.bind(this);
  ...
  onReady() {
    ...
    // Hide endcaps & tenons/tees by default
    document.querySelectorAll('#endcap-finish-container').forEach(container => {
  ...
  handleMountingChange(event) {
    // Only show endcap options if the selected mounting option is b2
    const mountingOption = event.target.value;
    const endcapContainer = document.querySelectorAll('#endcap-finish-container');
    if (mountingOption === 'b2') {
      endcapContainer.forEach(container => {
  ...
  handleOversizedGripChange(length, finish) {
    ...
    if ((length > 93 && finish.includes('stainless')) || (length > 81 && finish.includes('bronze'))) {
  ...
  handleSubmit(event, productSchema, getGripSize) {
    ...
    if (supportType && supportFinish) {
      ...
      // Get Tenon/Tee Product
      selectedSupport = this.productSchema[supportType][supportFinish];
      this.skuList.push({product: selectedSupport, quantity: totalGrips});
    } else {
      // Hide support information
      document.querySelector('#results--support-type-wrap').setAttribute('hidden', true);
      document.querySelector('#results--support-finish-wrap').setAttribute('hidden', true);
    }
    ...
    if (endcapFinish) {
      ...
      this.skuList.push({product: selectedEndcap, quantity: totalGrips * 2});
    } else {
      // Hide endcap information
      document.querySelector('#results--endcap-finish-wrap').setAttribute('hidden', true);
    }
    ...
    selectedEndcap && this.buildAndInsertResultTableRow(selectedEndcap, totalGrips * 2);
  ...
  bindListeners() {
    ...
    document.getElementById('grip-mount-selector').addEventListener('change', this.boundMountingChange);
