...
export default function (context) {
    function variantImageColor() {
        ...
        const self = $(this);
            const newImageVariant = self.data('image');
            const productItemElm = self.closest('.card');
    ...
    function headerPC() {
        ...
        } else if ($('.header').hasClass('is_sticky')) {
        ...
        $('.right-nav .mobileMenu-toggle .themevale_close').on('click', (ev) => {
        ...
        $('.search-nav .search-icon .themevale_close').on('click', (ev) => {
        ...
        $('.wrapper-top-cart .cart-icon .themevale_close').on('click', (ev) => {
    ...
    function toggleAccount_mobile() {
        $('.accountMobile').on('click', (e) => {
            $('body').addClass('themevale_open-Account');
        });

        $('#account-mobile .themevale_close2').on('click', (e) => {
            $('body').removeClass('themevale_open-Account');
        });

        $('.themevale_background').on('click', (e) => {
    ...
    function toggleCart_mobile() {
        $('.cartMobile').on('click', (e) => {
            $('body').addClass('themevale_open-Cart');
        });

        $('#cart-mobile .themevale_close2').on('click', (e) => {
    ...
    function toggleSearch_mobile() {
        $('.searchMobile').on('click', (e) => {
            $('body').addClass('themevale_open_search');
            searchProductsBlock();
        });

        $('#search-mobile .themevale_close2').on('click', (e) => {
            $('body').removeClass('themevale_open_search');
        });
    ...
    function carouselMegaMenu() {
        ...
        $(".navPages-list li").mouseover(() => {
            $('.featuredProductCarousel').get(0).slick.setPosition();
        });
        $(".navPages .navPages-action .navPages-action-moreIcon").on("click", () => {
            $('.featuredProductCarousel').get(0).slick.setPosition();
        });

        $('.featuredProductCarousel .product').each(function (index) {
            const title = $(this).find('.card-title > a').text().trim();
            if (title.length > 40) {
              const truncated = `${title.substring(0, 40).split(" ").slice(0, -1).join(" ")}…`;
    ...
    function searchProductsBlock() {
        const productIDS = context.themeSettings.search_product_block_id;
        const listIDs = JSON.parse(`[${productIDS}]`);

        for (let i = 0; i < listIDs.length; i++) {
             const productId = listIDs[i];

             if ($('.search_product_block .productGrid').length) {
               if ($('.search_product_block .productGrid li').length <= 0) {
                    utils.api.product.getById(productId, { template: 'products/quick-view' }, (err, response) => {
                        const data_product_id = $('.productView-title', $(response)).attr('data-product-id');
                        const brand = $('.productView-brand a', $(response)).text();
                        const name = $('.productView-title', $(response)).text();
                        const img = $('.productView-image', $(response)).find('img').attr('src');
                        const url = $('.productView-title', $(response)).data('url');
                        const price = $('.productView-price', $(response)).html();
    ...
    function countDownHeroCarousel() {
        const countDown = context.themeSettings.hero_carousel_countdown;
        const position = (parseInt(context.themeSettings.hero_carousel_countdown_position) + 1);
        const btn2 = context.themeSettings.hero_carousel_button_2;
    ...
    function initCountdown(id) {
        if ($('.countDowntimer').length) {
            ...
            const countDownDate = new Date($('.countDowntimer').attr('data-count-down')).getTime();
            ...
            var countdownfunction = setInterval(() => {
                ...
                if (distance < 0) {
                    ...
                    const strCountDown = `<div class='clock-item'><span class='num'>${days}</span><span class='text'>days</span></div><div class='clock-item'><span class='num'>${hours}</span><span class='text'>hours</span></div><div class='clock-item'><span class='num'>${minutes}</span><span class='text'>mins</span></div><div class='clock-item'><span class='num'>${seconds}</span><span class='text'>secs</span></div>`;
    ...
    function initCountdown2(id) {
        if ($('.countDowntimer2').length) {
            // Set the date we're counting down to
            const countDownDate2 = new Date($('.countDowntimer2').attr('data-count-down2')).getTime();

            // Update the count down every 1 second
            var countdownfunction2 = setInterval(() => {
                // Get todays date and time
                const now = new Date().getTime();
                ...
                if (distance2 < 0) {
                    ...
                    // Output the result in an element with id="countDowntimer"
                    const strCountDown2 = `<div class='clock-item'><span class='num'>${days}</span><span class='text'>D : </span></div><div class='clock-item'><span class='num'>${hours}</span><span class='text'>H : </span></div><div class='clock-item'><span class='num'>${minutes}</span><span class='text'>M : </span></div><div class='clock-item'><span class='num'>${seconds}</span><span class='text'>S</span></div>`;
    ...
    // Add To Wishlist
     $('body').on('click', '.card .wishlist', (e) => {
        e.preventDefault();
        const $this_wl = $(e.currentTarget);
        const url_awl = $this_wl.attr('href');
    ...
    function home_category_banner_block_slider(id_tab) {
        const column = context.themeSettings.homepage_carousel_products_column;
    ...
    $('#home_new_products ul li a').on("click", function () {
        ...
        const thisItem = $(this).parents('#home_new_products');
        let url = $(this).attr('cat-url');
        const cat_id = $(this).attr('cat-id');
        if ($(`#tab-${cat_id}`).find('.product_category').length == 0) {
          const sort_new = '?sort=newest';
          url += sort_new;
          if (url && thisItem.find(`#block-cat-${cat_id}`).length === 0) {
              ...
              utils.api.getPage(url, { template: 'themevale/homepage/home-category-content' }, (err, response) => {
                ...
                thisItem.find(`#tab-${cat_id}`).find('.card-img-container').each(function () {
                  const a = arrNew.indexOf($(this).data('product-id'));
                  if (a != -1) {
                ...
                if (context.themeSettings.themevale_color_variant == true) {
                    const block_id = thisItem.find('.tabs-contents .tab-content.is-active').attr('id');
    ...
    $('#home_new_products ul li a.all').on("click", () => {
    ...
    function home_custom_tab_products() {
        const thisItem = $("#homeTabsProducts");
        const url = thisItem.find('.tabs .custom-tab-title').attr('cat-url');
        ...
        if (thisItem.find('.tab-content-custom').length > 0) {
            ...
            utils.api.getPage(url, { template: 'themevale/homepage/home-category-content' }, (err, response) => {
                ...
                thisItem.find('#tab-custom').find('.card-figcaption-top').each(function () {
                  const a = arrNew2.indexOf($(this).data('product-id'));
                ...
                if (context.themeSettings.themevale_color_variant == true) {
                    const block_id = thisItem.find('.tabs-contents .tab-content').attr('id');
                    themevale_AddOption(context, block_id);
                }
    ...
    function showProductVariant($thisProd, $prodId) {
        const newProduct = context.themeSettings.homepage_carousel_products_count;
        if ($prodId != undefined) {
            const $options = {
                config: {
                    products: {
            ...
            utils.api.product.getById($prodId, $options, (err, response) => {
                ...
                const colorVariantToShow = 4;
                const productLink = $thisProd.find('.card-title > a').attr('href');

                // image swap
                if ($('[data-product-attribute="swatch"]', response.swap).length == 1) {
                    const $response = $(response.swap).find('.form-label--alternate, .form-radio, .form-option-expanded').remove().end();
                    const count_option = $('[data-product-attribute-value]',$('[data-product-attribute="swatch"]', response.swap)).length;
                    const more_option = (count_option - colorVariantToShow);
                    response.swap = $response;
                    let data_option = $('[data-product-attribute="swatch"]', response.swap).html();
                    ...
                    if (count_option > colorVariantToShow) {
                        $('[data-product-attribute-value]',$('[data-product-attribute="swatch"]', response.swap)).each(function (i) {
                            if (i >= colorVariantToShow) {
                                const option_id = $(this).attr('data-product-attribute-value');
                ...
                if ($('.Size[data-product-attribute="set-rectangle"]', response.swap).length == 1) {
                    const count_option2 = $('[data-product-attribute-value]',$('.Size[data-product-attribute="set-rectangle"]', response.swap)).length;
                    const more_option2 = (count_option2 - colorVariantToShow);
                    let data_option2 = $('.Size[data-product-attribute="set-rectangle"]', response.swap).html();
                    ...
                    if (count_option2 > colorVariantToShow) {
                        $('[data-product-attribute-value]',$('.Size[data-product-attribute="set-rectangle"]', response.swap)).each(function (i) {
                            if (i >= colorVariantToShow) {
                                const option_id2 = $(this).attr('data-product-attribute-value');
    ...
    function instagramShowMore() {
        const imageToShow = Number($('#InstagramGallery [data-show]').attr('data-show'));

        $('.infinite-scrolling-instagram .intagram-btn').on('click', function (e) {
            e.preventDefault();
            const listImages = $(this).parents('#InstagramGallery');
        ...
        if ($('#InstagramGallery #instafeed').length) {
            if ($('#instafeed .instagram-item').length > imageToShow) {
                ...
                for (let i = imageToShow + 1; i <= $('#instafeed .instagram-item').length; i++) {
    ...
    function recentlyBought() {
        const productIDs = context.themeSettings.recently_bought_productID;
        const Location1 = context.themeSettings.recently_bought_location1;
        const Location2 = context.themeSettings.recently_bought_location2;
        const Location3 = context.themeSettings.recently_bought_location3;
        const Location4 = context.themeSettings.recently_bought_location4;
        const Location5 = context.themeSettings.recently_bought_location5;
        const Location6 = context.themeSettings.recently_bought_location6;
        const Location7 = context.themeSettings.recently_bought_location7;
        const Location8 = context.themeSettings.recently_bought_location8;
        const Location9 = context.themeSettings.recently_bought_location9;
        const Location10 = context.themeSettings.recently_bought_location10;
        const ar1 = Location1.split(',');
        const ar2 = Location2.split(',');
        const ar3 = Location3.split(',');
        const ar4 = Location4.split(',');
        const ar5 = Location5.split(',');
        const ar6 = Location6.split(',');
        const ar7 = Location7.split(',');
        const ar8 = Location8.split(',');
        const ar9 = Location9.split(',');
        const ar10 = Location10.split(',');

        const hoursItems = context.themeSettings.recently_bought_hours;
        const listHours = JSON.parse(`[${hoursItems}]`);

        const listIDs = JSON.parse(`[${productIDs}]`);

        const text_info = context.themeSettings.recently_bought_text_info;
        const text_name = context.themeSettings.recently_bought_text_name;

        const changeSlides = context.themeSettings.recently_bought_changeSlides;
        const changeSlidesTime = 1000 * (Number(changeSlides));

        $(".themevale_popup").prepend('<div id="recently_bought_list"></div>');

        var recently = setInterval(() => {
            $('.themevale_recently-bought').hide();
            const item = (Math.floor(Math.random() * listIDs.length));
            const productId = listIDs[item];

            const locationList = Array(ar1,ar2,ar3,ar4,ar5,ar6,ar7,ar8,ar9,ar10);
            ...
            $(document).on('click','.themevale_recently-bought .modal-close',() => {
                $('#recently_bought_list').remove();
                clearInterval(recently);
            ...
            if ($(`#RB_${productId}`).length) {
                ...
                utils.api.product.getById(productId, { template: 'products/product-view' }, (err, response) => {
                    const name = $('.productView-product .productView-title', $(response)).text();
                    const img = $('.productView-image', $(response)).find('img').attr('src');
                    const url = $('.productView-title', $(response)).data('url');

                    const html = `<div id="RB_${productId}" class="themevale_recently-bought">\
    ...
    } else if (context.themeSettings.recently_bought == true && context.themeSettings.show_recently_bought_mobile == true) {
            recentlyBought();
        }
    ...
    function lookbook() {
        ...
        $('.lookbook-item .position-point').on('click', function () {
            ...
            const iconWidth = $(this).innerWidth();
            const innerLookbookModal = $('.lookbook-popup').innerWidth();
            const str3 = `${iconWidth}px`;
            const str4 = `${innerLookbookModal}px`;
