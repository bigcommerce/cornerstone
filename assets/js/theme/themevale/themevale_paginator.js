...
export default function(context){
	var galleryPerPage = context.themeSettings.webpage_gallerys_per_page;
	...
	theme.Pagination = function() {
		...
		function Pagination(){
			...
			$(document).on('click', '.pagination-link', function(e){
				...
				if( $(this).data('page') ){
					...
					var end = _.current_page * _.per_page,
						start = end - _.per_page;
		...
		Pagination.prototype = $.extend({}, Pagination.prototype, {
			...
			show_item: function(start, end){
				$('.image-gallery-item').addClass('hide');
				$('.image-gallery-item').each(function(i, el){
					if( i >= start && i < end )
						$(el).removeClass('hide');
				});
			},
			show_pagination: function( current_page ) {
				$('.themevale-pagination .pagination-list').html('');
				for (var i = 0; i < _.total_page; i++) {
					if( i + 1 == current_page ) {
